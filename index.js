...
const main = async () => {
  ...
  try {
    ...
    const repository_root = core.getInput('repository_root', {
      required: true,
    });
    const branch = core.getInput('branch', { required: true });
    const cpanel_token = core.getInput('cpanel_token', { required: true });
    const cpanel_username = core.getInput('cpanel_username', {
      required: true,
    });
    ...
    const createDeploymentTaskEndpoint =
      baseUrl + '/VersionControlDeployment/create';
    ...
    if (startDeployRes.errors !== null) {
      // noinspection ExceptionCaughtLocallyJS
      throw new Error(
        'Failed to start deployment task: ' +
          JSON.stringify(startDeployRes.errors, null, 2)
      );
    }
    ...
    for (let i = 0; i < maxWaitSeconds; i++) {
      ...
      const taskData = pollRes.data.filter(
        (info) => info.task_id === taskId
      )[0];
      ...
      core.debug(
        `task ${taskId} still running. taskData: ${JSON.stringify(
          taskData,
          null,
          2
        )}`
